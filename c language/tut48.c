...
int main()
{
    ...
    while (i < 3)
    {
        printf("plz enter character of employee id of \n");
        scanf("%d", &chars);
        ptr = (char *)malloc((chars + 1) * sizeof(char));
        printf("Enter your %d Employee ID is\n",  i+1);
        scanf("%s", ptr); //we do not use '&' because ptr is already address variable;

        printf("You %d Employee is:%s\n", i+1, ptr);
